 ...
 * correctly balanced.  Returns 0 for success, -1 otherwise.
 */
				  ...
				  void *addr, size_t count)
{
	...
	if (dma_mapping_error(parport_ip32_dma.buf))
		return -1;
 ...
 */
static int parport_ip32_dma_register(struct parport *p)
{
	int err;
 ...
 */
static void parport_ip32_dma_unregister(struct parport *p)
{
	free_irq(MACEISA_PAR_CTXB_IRQ, p);
	free_irq(MACEISA_PAR_CTXA_IRQ, p);
}
						...
						const void *buf, size_t len)
{
	...
	err = parport_ip32_dma_start(DMA_TO_DEVICE, (void *)buf, len);
	if (err) {
		printk(KERN_WARNING PPIP32
		       "%s: DMA failed to start, reverting to PIO mode",
			p->name);
		priv->irq_mode = PARPORT_IP32_IRQ_FWD;
		written = parport_ip32_fifo_write_block_pio(p, buf, len);
		return written;
	}
	INIT_COMPLETION(priv->irq_complete);
 ...
 * @pdev:	pointer to the &struct parport_device
 *
 * Probing routine for the platform driver.
 */
static __init int parport_ip32_probe_port(struct platform_device *pdev)
{
	struct parport_ip32_regs regs;
	...
	*priv = (struct parport_ip32_private){
	...
	if (features & PARPORT_IP32_ENABLE_DMA) {
		if (parport_ip32_dma_register(p))
			printk(KERN_WARNING PPIP32
...
/**
 * parport_ip32_remove_port - remove the parallel port
 * @pdev:	pointer to the &struct parport_device
 *
 * Removing routine for the platform driver.
 */
static __exit int parport_ip32_remove_port(struct platform_device *pdev)
{
	struct parport *p = pdev->dev.platform_data;
	struct parport_ip32_private * const priv = p->physport->private_data;
	struct parport_operations *ops = p->ops;

	parport_remove_port(p);
	if (p->modes & PARPORT_MODE_DMA)
		parport_ip32_dma_unregister(p);
	if (p->irq != PARPORT_IRQ_NONE)
		free_irq(p->irq, p);
	parport_put_port(p);
	kfree(priv);
...
static struct platform_driver parport_ip32_platform_driver = {
	.driver = {
		.owner = THIS_MODULE,
		.name  = "parport_ip32",
	},
	.remove = parport_ip32_remove_port,
};
...
{
	int err;
	pr_info(PPIP32 "SGI IP32 built-in parallel port driver v%s\n",
		DRV_VERSION);
	pr_debug1(PPIP32 "Compiled on %s, %s\n", __DATE__, __TIME__);
	err = platform_driver_probe(&parport_ip32_platform_driver,
				    parport_ip32_probe_port);
	if (err)
		printk(KERN_ERR PPIP32 "Driver registration failed\n");
	return err;
}
...
{
	platform_driver_unregister(&parport_ip32_platform_driver);
}
